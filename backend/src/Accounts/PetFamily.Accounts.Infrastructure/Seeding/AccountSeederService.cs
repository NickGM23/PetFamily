...
namespace PetFamily.Accounts.Infrastructure.Seeding
{
        ...
        ILogger<AccountsSeeder> logger)
    {
        ...
        private async Task SeedAdminAsync(CancellationToken cancellationToken = default)
        {
            ...
            var fullName = FullName.Create(adminOptions.Value.LastName,
                adminOptions.Value.FirstName,
        ...
        private async Task SeedPermissionsAsync(
            RolePermissionOptions options,
            CancellationToken cancellationToken = default)
        {
            logger.LogInformation("Seeding permissions...");

            var permissions = options.Permissions.SelectMany(pg => pg.Value);
